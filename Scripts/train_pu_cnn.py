...
def get_predictions(model, dataloader, device):
    ...
    with torch.no_grad():
        for x, _ in tqdm(dataloader, desc="  Predicting", leave=False):
            x = x.to(device)
            outputs = model(x)
            probs.extend(outputs.cpu().float().numpy().flatten())
...
def run_pu_learning(label_key='l99'):
    ...
    model_filename = f"cnn_pu_paper_{label_key}.pth"
    MODEL_SAVE_PATH = os.path.join(BASE_DIR, "Models", model_filename)
    
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Using device: {device}")
    print(f"Training CNN PU model for label: {label_key}")
    ...
    SEQ_MAX_LEN = 1024 
    model = DualStrandCNN(seq_length=SEQ_MAX_LEN).to(device)
    ...
    for k in range(max_k):
        ...
        train_ds = GenomicDataset(train_data, use_onehot=True, max_length=SEQ_MAX_LEN)
        ...
        p_ds = GenomicDataset(current_P, use_onehot=True, max_length=SEQ_MAX_LEN)
        p_loader = DataLoader(p_ds, batch_size=128, shuffle=False)
        ...
        unlabeled_ds = GenomicDataset(current_U, use_onehot=True, max_length=SEQ_MAX_LEN)
        unlabeled_loader = DataLoader(unlabeled_ds, batch_size=128, shuffle=False)
    ...
    test_ds = GenomicDataset(test_samples, use_onehot=True, max_length=SEQ_MAX_LEN)
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Run PU Learning for CNN")
