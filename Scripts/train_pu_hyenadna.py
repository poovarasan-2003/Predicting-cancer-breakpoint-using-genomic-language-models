...
def run_pu_learning(label_key='l99'):
    ...
    model_filename = f"hyenadna_pu_paper_{label_key}.pth"
    MODEL_SAVE_PATH = os.path.join(BASE_DIR, "Models", model_filename)
    
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Using device: {device}")
    print(f"Training HyenaDNA PU model for label: {label_key}")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Run PU Learning for HyenaDNA")
