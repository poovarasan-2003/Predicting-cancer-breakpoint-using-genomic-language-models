...
def parse_arguments():
    parser = argparse.ArgumentParser(description='Local Nucleotide Transformer Training with SHAP Analysis for Fragile Site Detection')
    ...
    parser.add_argument('--model_name', type=str,
                       default="local_nucleotide_transformer_500m",
                       help='Model name (using local architecture)')
    parser.add_argument('--embed_dim', type=int, default=1280,
                       help='Embedding dimension (default: 1280 for 500M model)')
    ...
    parser.add_argument('--num_layers', type=int, default=30,
                       help='Number of transformer layers (default: 30)')
    parser.add_argument('--ffn_dim', type=int, default=5120,
                       help='Feed-forward network dimension (default: 5120)')
    ...
    parser.add_argument('--use_glu', action='store_true',
                       help='Use GLU activation in feed-forward network')
    parser.add_argument('--use_rotary_embedding', action='store_true',
    ...
    parser.add_argument('--model_save_name', type=str, default='fragile_sites_local_nt_model.pth',
                       help='Model save filename (default: fragile_sites_local_nt_model.pth)')
...
# ===== CUSTOM NUCLEOTIDE TRANSFORMER ARCHITECTURE =====
...
class FeedForwardNetwork(nn.Module):
    ...
    def __init__(self, embed_dim, ffn_dim, dropout=0.1, use_glu=False):
        super().__init__()
        self.use_glu = use_glu
        
        if use_glu:
            self.fc1 = nn.Linear(embed_dim, 2 * ffn_dim)
        else:
...
class TransformerBlock(nn.Module):
    ...
    def __init__(self, embed_dim, num_heads, ffn_dim, dropout=0.1, 
                 layer_norm_eps=1e-5, use_glu=False):
        super().__init__()
        
        self.attention = MultiHeadAttention(embed_dim, num_heads, dropout)
        self.feed_forward = FeedForwardNetwork(embed_dim, ffn_dim, dropout, use_glu)
...
class DNATokenizer:
    """Simple DNA tokenizer for ACTGN sequences"""
    def __init__(self, vocab_size=8, max_length=512):
    ...
    def __call__(self, sequences, max_length=None, padding='max_length', 
        ...
        for seq in sequences:
            ...
            for nucleotide in seq[:max_length-2]:  # Leave room for CLS and SEP
                token_ids.append(self.vocab.get(nucleotide.upper(), self.vocab['N']))
            ...
            encoded_sequences.append(token_ids[:max_length])
...
class NucleotideTransformerModel(nn.Module):
    """
    Custom Nucleotide Transformer Model Architecture
    Based on the 500M parameter model configuration
    """
    def __init__(
        self,
        vocab_size=8,
        ...
        self.use_learned_pos_embedding = use_learned_pos_embedding
        self.use_rotary_embedding = use_rotary_embedding
        
        if use_learned_pos_embedding:
            self.position_embedding = nn.Embedding(max_positions, embed_dim)
        elif use_rotary_embedding:
            self.rotary_emb = RotaryPositionalEmbedding(embed_dim // num_heads)
        else:
            # Sinusoidal positional encoding
            self.register_buffer('pos_encoding', self._create_sinusoidal_positions(max_positions, embed_dim))
            
        # Layer normalization before transformer blocks
        self.emb_layer_norm = nn.LayerNorm(embed_dim, eps=layer_norm_eps)
    ...
    def _create_sinusoidal_positions(self, max_positions, embed_dim):
        """Create sinusoidal positional embeddings"""
        position = torch.arange(max_positions).unsqueeze(1)
        div_term = torch.exp(torch.arange(0, embed_dim, 2) * 
                            -(math.log(10000.0) / embed_dim))
        
        pos_embedding = torch.zeros(max_positions, embed_dim)
    ...
    def forward(self, input_ids, attention_mask=None):
        ...
        # Pool the outputs (mean pooling over sequence length)
        if attention_mask is not None:
        ...
        # Return in format compatible with HuggingFace models
        class Output:
            def __init__(self, logits, last_hidden_state=None):
                self.logits = logits
                self.last_hidden_state = last_hidden_state
                
        return Output(logits=logits, last_hidden_state=x)

class LocalNucleotideTransformerWithEmbeddings(nn.Module):
    """Wrapper to extract embeddings from the local Nucleotide Transformer model"""
    def __init__(self, config):
        super().__init__()
        self.base_model = NucleotideTransformerModel(**config)
...
class SegmentEnformerNTModelWithEmbeddings(torch.nn.Module):
    """DEPRECATED - Using LocalNucleotideTransformerWithEmbeddings instead"""
    pass

# Embedding visualization functions
def extract_embeddings_from_dataloader(model, dataloader, device, max_samples=None):
    """Extract embeddings from a dataloader using Local Nucleotide Transformer model"""
    model.eval()
    all_embeddings = []
    all_labels = []
    all_predictions = []
...
def create_embedding_visualizations(embeddings, labels, predictions=None, 
                                   ...
                                   sample_names=None):
    """Create visualization plots for embeddings (Local Nucleotide Transformer titles)"""
    ...
    # Updated suptitle (Local Nucleotide Transformer wording)
    plt.suptitle('Nucleotide Transformer Model Embedding Space Visualization', fontsize=18, fontweight='bold', y=1.02)
    plt.tight_layout()  
    ...
    for method in methods:
        ...
        # Updated suptitle for Local Nucleotide Transformer wording
        plt.suptitle(f'{method} Visualization - Nucleotide Transformer DNA Fragile Sites Model', 
...
def run_visualization_pipeline(model, train_loader, val_loader, test_loader, 
    ...
    print("\n" + "="*70)
    print("STARTING EMBEDDING VISUALIZATION PIPELINE - NUCLEOTIDE TRANSFORMER MODEL")
    print("="*70)
...
def create_combined_dataset_plot(all_results, save_dir):
    ...
    for method in methods:
        ...
        plt.suptitle(f'Nucleotide Transformer Model - {method} Embeddings Across Datasets', 
...
class LocalNucleotideTransformerModelWrapper:
...
def create_shap_html_visualization(model, tokenizer, sequence, output_dir, max_length=256):
    """
    Create SHAP HTML visualization for Local Nucleotide Transformer model
    """
    ...
    print("\n" + "="*60)
    print("CREATING SHAP HTML VISUALIZATION (Nucleotide Transformer)")
    print("="*60)
    
    # Create model wrapper for HTML visualization
    class LocalNTHTMLWrapper:
    ...
    wrapper = LocalNTHTMLWrapper(model, tokenizer, max_length, device)
    ...
    html_path = os.path.join(output_dir, "local_nt_shap_dna_visualization.html")
    ...
    try:
        ...
        print(f"✔ SHAP visualization saved to: local_nt_shap_dna_visualization.html")
        return True
        
    except Exception as e:
        print(f"Creating custom HTML visualization: {e}")
        create_custom_local_nt_html(
            sequence_to_analyze,
            importance_scores,
            baseline_score,
            html_path,
            model_name="Local Nucleotide Transformer"
        )
        print(f"Custom HTML saved to: local_nt_shap_dna_visualization.html")
        return True

def plot_roc_auc_curve(y_true, y_scores, save_path="roc_auc_curve.png", model_name="Local Nucleotide Transformer"):
    """
...
def create_custom_local_nt_html(sequence, importance_scores, baseline_score, output_path, model_name="Local Nucleotide Transformer"):
    """
    Create custom HTML visualization for Local Nucleotide Transformer
    """
    import numpy as np
    import os  
    ...
    # Create HTML with Local Nucleotide Transformer-specific styling (blue gradient theme)
    html_content = f"""
    <!DOCTYPE html>
    <html>
    <head>
...
def main():
    ...
    print("STARTING LOCAL NUCLEOTIDE TRANSFORMER TRAINING WITH SHAP ANALYSIS")
    print("=" * 70)
    ...
    # Loading the Local Nucleotide Transformer Model
    print("INITIALIZING LOCAL NUCLEOTIDE TRANSFORMER MODEL")
    print("=" * 70)
    
    # Define model configuration
    model_config = {
        'vocab_size': 8,  # DNA vocabulary size
        'max_positions': 1024,
    ...
    model = LocalNucleotideTransformerWithEmbeddings(model_config)
    
    # Initialize the DNA tokenizer
    tokenizer = DNATokenizer(vocab_size=8, max_length=args.max_length)
    ...
    if args.enable_shap:
        ...
        try:
            ...
            if html_success:
                ...
                print(f"  • SegmentEnformerNT: {args.output_dir}/SegmentEnformerNT_shap_dna_visualization.html")
